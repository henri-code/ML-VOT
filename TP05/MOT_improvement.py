...
def draw_tracking_results(image_dir, tracks, iou_threshold=0.5):
    ...
    for idx, image_file in enumerate(image_files):
        ...
        if cv2.waitKey(100) == 27:  # Attendre 100 ms entre chaque image et vérifier si 'echap' est pressé pour quitter
            break
...
def extract_appearance_embedding(image_path, model):
    image_path = 'TP02/ADL-Rundle-6/img1/' + '000' + format_number(image_path) + '.jpg'
...
def track_management_with_hungarian_and_kalman_filter_and_ia(tracks, detections, similarity_matrix, appearance_model):
    ...
    detection_appearances = [extract_appearance_embedding(detection['frame'], appearance_model) for detection in detections]
    ...
    for track in tracks:
        if track['id'] in assignment_dict:
            ...
            track_appearance = extract_appearance_embedding(track['last_detection']['frame'], appearance_model)

            # Calculer la similarité cosinus entre l'embedding de la piste et celui de la détection
            visual_similarity = cosine_similarity([track_appearance], [detection_appearances[detection_idx]])[0][0]
            ...
            cost_matrix[detection_idx, track['id']] += visual_similarity  # Ajouter la similarité visuelle au coût existant
    ...
    for col_idx in range(num_detections):
        if col_idx not in assignment_dict.values():
            ...
            detection_appearance = extract_appearance_embedding(detection['frame'], appearance_model)

            # Créer une nouvelle piste avec les prédictions du filtre de Kalman et la similarité visuelle
            new_track = {'id': num_tracks + col_idx, 'detections': [detection], 'kalman_state': kalman_state, 'visual_similarity': 0}
            updated_tracks.append(new_track)
